/**
 * @file basic_stencils/fpga_kernels/kernel_body/4d8opt.h
 * @brief File to be included multiple times inside 4d8opt.cl, to avoid repetition of hard-to-debug
 * code.
 *
 * This file expects x_size, y_size, z_size and w_size to be defined int variables in the file
...
// Macro to signal that more than one block in the second dimension will be computed,
// manages certain logic
#ifndef MORE_THAN_ONE_Y_BLOCK
#define MORE_THAN_ONE_Y_BLOCK true
#endif // MORE_THAN_ONE_Y_BLOCK

// Macro to signal that more than one block in the third dimension will be computed,
// manages certain logic
#ifndef MORE_THAN_ONE_Z_BLOCK
#define MORE_THAN_ONE_Z_BLOCK true
#endif // MORE_THAN_ONE_Y_BLOCK
...
int block_shift = HALO_BLOCK_WIDTH_X * HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_Z * w_size + SR_BASE;
// HALO_BLOCK_WIDTH_X * HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_Z * w_size to process all the elements in the block
...
int z  = -RAD;     // z in the block; range: [-RAD, BLOCK_WIDTH_Z + RAD),
int gz = z;        // z in the global matrix
int gw = -2 * RAD; // w in the global matrix (offset to initialize the shift register each block)
				   // range: [-2 * RAD, w_size)
while (shift_idx < total_to_shift) {
	...
	for (int vec_idx = 0; vec_idx < VEC_SIZE; vec_idx++) {
		// Read coords: The element read is exactly the one +w the current center
		int vgx_to_read = gx + vec_idx;
		int vgy_to_read = gy;
		int vgz_to_read = gz;
		int vgw_to_read = gw + RAD;

		#ifndef DEBUG
		shift_register[SR_BASE + vec_idx] = HIT(matrixCopy, vgw_to_read, vgz_to_read, vgy_to_read, vgx_to_read);
		// Incorrect (out of bounds) accesses will be discarded later
		#else // DEBUG
		if (vgx_to_read < x_size + RAD && vgy_to_read < y_size + RAD && vgz_to_read < z_size + RAD)
			shift_register[SR_BASE + vec_idx] = HIT(matrixCopy, vgw_to_read, vgz_to_read, vgy_to_read, vgx_to_read);
		else
	...
	for (int vec_idx = 0; vec_idx < VEC_SIZE; vec_idx++) {
		...
		bool shift_register_initialized = gw >= 0;
		...
		bool   z_in_bounds = z >= 0 && z < BLOCK_WIDTH_Z && (!MORE_THAN_ONE_Z_BLOCK || gz < z_size);
		bool   in_bounds   = x_in_bounds && y_in_bounds && z_in_bounds;
		...
		if (shift_register_initialized && in_bounds)
			HIT(matrix, gw, gz, gy, vgx) = (sr_center[-HALO_BLOCK_WIDTH_Z * HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_X] +
											sr_center[+HALO_BLOCK_WIDTH_Z * HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_X] +
	...
	if (x == -RAD) {              // End of row
		...
		if (y == -RAD) {              // End of slice
			...
			z = (z + 1 + RAD) % HALO_BLOCK_WIDTH_Z - RAD;
			if (z == -RAD) {              // End of cube
				gz -= HALO_BLOCK_WIDTH_Z; // Reset global z coord
				gw++;                     // Move to next plane
				if (gw == w_size) {       // End of cube
					gw = -2 * RAD;        // Offset to initialize the shift register
					gx += BLOCK_WIDTH_X;  // Move to next x block
					if (MORE_THAN_ONE_Y_BLOCK) {
						if (gx >= x_size) { // End of X blocks
							...
							if (MORE_THAN_ONE_Z_BLOCK && gy >= y_size) {
								// (The comparison above is ge and not eq because the real Y size of the
								// array is not known and can be non-multiple of HALO_BLOCK_WIDTH_Y.)
								gy = -RAD;           // Reset global y coord to the beginning of the column
								gz += BLOCK_WIDTH_Z; // Move to next z block
							}
						}
					} else if (MORE_THAN_ONE_Z_BLOCK && gx >= x_size) {
						// (The comparison above is ge and not eq because the real Y size of the
						// array is not known and can be non-multiple of HALO_BLOCK_WIDTH_Y.)
						gx = -RAD;           // Reset global x coord to the beginning of the column
						gz += BLOCK_WIDTH_Z; // Move to next z block
					}
				}
			}

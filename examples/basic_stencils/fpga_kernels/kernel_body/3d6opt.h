/**
 * @file basic_stencils/fpga_kernels/kernel_body/3d6opt.h
 * @brief File to be included multiple times inside 3d6opt.cl, to avoid repetition of hard-to-debug
 * code.
 *
...
while (shift_idx < total_to_shift) {
	...
	for (int vec_idx = 0; vec_idx < VEC_SIZE; vec_idx++) {
		// Read coords: The element read is exactly the one below the current center
	...
	for (int vec_idx = 0; vec_idx < VEC_SIZE; vec_idx++) {
		...
		if (shift_register_initialized && in_bounds)
			HIT(matrix, gz, gy, vgx) = (sr_center[-HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_X] +

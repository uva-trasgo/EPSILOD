/**
 * @file basic_stencils/fpga_kernels/kernel_body/2dNC8opt.h
 * @brief File to be included multiple times inside 2dNC8opt.cl, to avoid repetition of hard-to-debug code.
 *
...
while (shift_idx < total_to_shift) {
	...
	for (int vec_idx = 0; vec_idx < VEC_SIZE; vec_idx++) {
		// Read coords: The element read is the southmost neighbor of the current center
	...
	for (int vec_idx = 0; vec_idx < VEC_SIZE; vec_idx++) {
		...
		if (shift_register_initialized && in_bounds)
			hit(matrix, gy, vgx) = (sr_center[-2 * HALO_BLOCK_WIDTH] +
									sr_center[+2 * HALO_BLOCK_WIDTH] +
									sr_center[-2] +
									sr_center[+2] +
									4 * (sr_center[-HALO_BLOCK_WIDTH] +

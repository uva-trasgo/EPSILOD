/**
 * @file basic_stencils/fpga_kernels/kernel_body/3d27opt.h
 * @brief File to be included multiple times inside 3d27opt.cl, to avoid repetition of hard-to-debug
 * code.
 *
...
// Indeces:
int x = -RAD + COMPUTE_X_OFFSET;  // x in the block; range: [-RAD, BLOCK_WIDTH_X + RAD),
								  // the range is multiple of VEC_SIZE, to have aligned accesses
int gx = x;                       // x in the global matrix
int y  = -RAD + COMPUTE_Y_OFFSET; // y in the block; range: [-RAD, BLOCK_WIDTH_Y + RAD),
...
while (shift_idx < total_to_shift) {
	...
	for (int vec_idx = 0; vec_idx < VEC_SIZE; vec_idx++) {
		// Read coords: The element read is the one southeast-below the current center
	...
	for (int vec_idx = 0; vec_idx < VEC_SIZE; vec_idx++) {
		...
		if (shift_register_initialized && in_bounds)
			HIT(matrix, gz, gy, vgx) = (sr_center[-HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_X - HALO_BLOCK_WIDTH_X - 1] +
										sr_center[-HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_X - HALO_BLOCK_WIDTH_X] +
										sr_center[-HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_X - HALO_BLOCK_WIDTH_X + 1] +
										sr_center[-HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_X - 1] +
										sr_center[-HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_X] +
										sr_center[-HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_X + 1] +
										sr_center[-HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_X + HALO_BLOCK_WIDTH_X - 1] +
										sr_center[-HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_X + HALO_BLOCK_WIDTH_X] +
										sr_center[-HALO_BLOCK_WIDTH_Y * HALO_BLOCK_WIDTH_X + HALO_BLOCK_WIDTH_X + 1] +
										sr_center[-HALO_BLOCK_WIDTH_X - 1] +
	...
	// (This current logic works with all thought scenarios because the compute x/y offsets
	// is always equal to the size of the halo (i.e., non-computed part) at the right/south, so
	...
	gy += COMPUTE_Y_OFFSET;
	y += COMPUTE_Y_OFFSET;
	// The compute coords corresponding to the "compute" of the right and bottom halo are
	// substituted  by other, invalid coords (-2 * RAD), so their compute is skipped anyway.
}

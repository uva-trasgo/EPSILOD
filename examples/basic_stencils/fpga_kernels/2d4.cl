/**
 * @file basic_stencils/fpga_kernels/2d4.cl
...
CTRL_KERNEL_FN(updateCell_2d4, FPGA, NDRANGE, KHitTileR_arg(float, matrix), KHitTileR_arg(float, matrixCopy),
...
CTRL_KERNEL_FN(updateCell_2d4_xBorder, FPGA, NDRANGE, KHitTileR_arg(float, matrix), KHitTileR_arg(float, matrixCopy),
			   K_arg(EpsilodCoords, global_coords), KHitTileR_arg(float, stencil), K_arg(float, factor),
			   K_arg(Epsilod_ext, ext_params)) {
	hit(matrix, thr_i, thr_j) = (hit(matrixCopy, thr_i - 1, thr_j) +
								 hit(matrixCopy, thr_i + 1, thr_j) +
...
CTRL_KERNEL_FN(updateCell_2d4_yBorder, FPGA, NDRANGE, KHitTileR_arg(float, matrix), KHitTileR_arg(float, matrixCopy),
			   K_arg(EpsilodCoords, global_coords), KHitTileR_arg(float, stencil), K_arg(float, factor),
			   K_arg(Epsilod_ext, ext_params)) {
	hit(matrix, thr_i, thr_j) = (hit(matrixCopy, thr_i - 1, thr_j) +
								 hit(matrixCopy, thr_i + 1, thr_j) +

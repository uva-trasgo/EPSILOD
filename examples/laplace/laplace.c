	...
	if (hit_sigIn(hit_tileDimSig(tileMat, 1), hit_tileDimBegin(root, 1)))
		for (j = 0; j < global.borders.low[1]; j++)
			hit_tileForDimDomain(tileMat, 0, i)
				hit_tileElemAt(tileMat, 2, i, j) = 0;

	/* 2.2. LAST COLUMN IS MINE */
	if (hit_sigIn(hit_tileDimSig(tileMat, 1), hit_tileDimEnd(root, 1)))
		for (j = 0; j < global.borders.high[1]; j++)
			hit_tileForDimDomain(tileMat, 0, i)
				hit_tileElemAt(tileMat, 2, i, hit_tileDimCard(tileMat, 1) - 1 - j) = (EPSILOD_BASE_TYPE)i / (size.x - 1);
...
void outputData(HitTile(EPSILOD_BASE_TYPE) io_tile, Epsilod_ext *ext_params) {
	...
	hit_tileFileWriteOptions(&io_tile, output_file_name, NULL, HIT_FILE_RUNTIME, HIT_FILE_BINARY, HIT_FILE_ARRAY, HIT_FILE_NO_HEADER, HIT_FILE_TYPE_UNKNOWN, 0, 0);
}

/* D. DECLARATIONS OF OPTIMIZED STENCIL KERNEL
 * SEE laplace_kernel.c FILE */
REGISTER_STENCIL(updateCell_laplace, GENERIC, DEFAULT);
REGISTER_STENCIL(noop_laplace, GENERIC, DEFAULT);
...
int main(int argc, char *argv[]) {
	...
	/* RADIUS */
	int      radius              = 1;
	HitShape shp_stencil_laplace = hitShape((-radius, radius), (-radius, radius));

	/* WEIGHTS: SPECIALIZED KERNEL, VALUES ARE USED ONLY TO COMPUTE BORDERS */
	float stencilData_laplace[] = {
		0, 1, 0,
		1, 0, 1,
		0, 1, 0};
	...
	stencilComputation(sizes, shp_stencil_laplace, stencilData_laplace, 1.0f, iterations, initData, NULL, NULL, f_stencil, outputData, &ext_params, device_selection_file);
